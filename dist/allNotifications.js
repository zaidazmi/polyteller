...
/* harmony import */ var _utils_timezoneUtils__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../utils/timezoneUtils */ "./src/utils/timezoneUtils.ts");
/* harmony import */ var _utils_CountdownManager__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ../utils/CountdownManager */ "./src/utils/CountdownManager.ts");
/* harmony import */ var _utils_countdownFormatters__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../utils/countdownFormatters */ "./src/utils/countdownFormatters.ts");
...
function displayAllNotifications() {
    ...
    chrome.runtime.sendMessage({ type: 'GET_STORED_NOTIFICATIONS' }, (response) => {
        ...
        const events = _store_store__WEBPACK_IMPORTED_MODULE_0__.useStore.getState().events;
...
function groupNotificationsByEvent(notifications, eventMap) {
...
function createEventElement(event, notifications) {
  ...
  `;
    ...
    if (notificationsList) {
        notifications.forEach(notification => {
            ...
            const localNotificationTime = (0,_utils_timezoneUtils__WEBPACK_IMPORTED_MODULE_4__.convertToLocalTime)(notificationTime);
        ...
        <div class="notification-info">
          ...
          <span class="notification-date">${(0,_utils_timezoneUtils__WEBPACK_IMPORTED_MODULE_4__.formatLocalTime)(localNotificationTime)}</span>
        </div>
...
async function deleteNotification(event) {
    ...
    const button = target.closest('.delete-notification');
    if (!button)
        return;
    const eventId = button.getAttribute('data-event-id');
    const minutesBefore = parseFloat(button.getAttribute('data-minutes-before') || '0');
    (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_2__.log)('All Notifications', `Attempting to delete notification: eventId=${eventId}, minutesBefore=${minutesBefore}`);
    if (eventId && !isNaN(minutesBefore)) {
        const deletedNotification = { eventId, minutesBefore };
        (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_2__.log)('All Notifications', `Notification to delete:`, deletedNotification);
        try {
            const response = await chrome.runtime.sendMessage({
                type: 'REMOVE_NOTIFICATION_ALARM',
                data: deletedNotification
            });
            (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_2__.log)('All Notifications', `Received response from background:`, response);
            if (response.success) {
                _store_store__WEBPACK_IMPORTED_MODULE_0__.useStore.getState().removeNotification(eventId, minutesBefore);
                await displayAllNotifications();
                (0,_utils__WEBPACK_IMPORTED_MODULE_3__.displayStatus)('Notification deleted successfully!');
            }
            else {
                (0,_utils__WEBPACK_IMPORTED_MODULE_3__.displayStatus)('Error deleting notification. Please try again.');
            }
        }
        catch (error) {
            (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_2__.log)('All Notifications', 'Error sending message to background:', error);
            (0,_utils__WEBPACK_IMPORTED_MODULE_3__.displayStatus)('Error communicating with background. Please try again.');
        }
    }
    else {
        (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_2__.log)('All Notifications', 'Invalid notification data for deletion');
        (0,_utils__WEBPACK_IMPORTED_MODULE_3__.displayStatus)('Error: Invalid notification data');
    }
}
function startCountdowns() {
    ...
    countdownElements.forEach(element => {
        ...
        updateCountdown(element, eventId, endTime);
    });
}
function updateCountdown(element, eventId, endTime) {
    const countdownManager = _utils_CountdownManager__WEBPACK_IMPORTED_MODULE_5__.CountdownManager.getInstance();
    countdownManager.registerEvent({ id: eventId, endTime });
    return countdownManager.subscribe(eventId, (timeLeft) => {
      ...
      `;
            ...
            element.innerHTML = (0,_utils_countdownFormatters__WEBPACK_IMPORTED_MODULE_6__.formatAllNotificationsCountdown)(timeLeft);
        }
    });
}
// Store unsubscribe functions
const countdownUnsubscribes = new Map();
// Cleanup function
...
/******/ })()
;
//# sourceMappingURL=allNotifications.js.map