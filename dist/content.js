...
function checkEventResolution() {
    try {
        ...
        (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_3__.log)('Content', 'Error checking resolution status:', error);
        return { isResolved: false };
    }
}
function extractEventInfo() {
    ...
    try {
        ...
        if (eventData && eventData.title) {
            ...
            (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_3__.log)('Content', 'Resolution status:', { isResolved, outcome });
            let timezone = 'ET';
...
const DATE_PATTERNS = [
    ...
    {
        name: 'TIME_WITH_AT_FORMAT',
        pattern: /([A-Za-z]+\s+\d{1,2},\s*\d{4})\s*at\s*(\d{1,2}):(\d{2})\s*([AP])M\s*([A-Z]{2})/i,
        priority: 195,
        format: "Month DD, YYYY at HH:MM AM/PM ET",
        handler: _patternHandlers__WEBPACK_IMPORTED_MODULE_0__.handleTimeWithAtFormat
    }
...
/* harmony export */   handleTimeWithAtFormat: () => (/* binding */ handleTimeWithAtFormat),
 ...
 */
function handleTimeWithAtFormat(match) {
    ...
    (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_0__.log)('Content', 'Matched time with at format:', {
        datePart,
        hour,
        minute,
        ampm,
