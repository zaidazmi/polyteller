...
async function checkMissedAlarms() {
    try {
        ...
        (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_0__.log)('Background', 'Error checking missed alarms:', error);
    }
}
/**
...
(() => {
    ...
    chrome.runtime.onUpdateAvailable.addListener((details) => {
        (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_1__.log)('Background', `Update available: ${details.version}`);
    ...
    chrome.runtime.onInstalled.addListener((details) => {
        if (details.reason === 'update') {
            (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_1__.log)('Background', `Updated from ${details.previousVersion} to ${chrome.runtime.getManifest().version}`);
        ...
        try {
            ...
            if (request.action === 'updatePrivacyMode') {
                handleUpdatePrivacyMode({
                    type: _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.UPDATE_PRIVACY_MODE,
            ...
            if (request.action === 'broadcastPrivacyMode') {
                handleBroadcastPrivacyMode({
                    type: _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.BROADCAST_PRIVACY_MODE,
                    data: { enabled: request.enabled },
                    requestId: Date.now().toString(),
                    timestamp: Date.now()
                }, sendResponse);
            ...
            switch (request.type) {
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.EVENT_INFO:
                    handleEventInfo(request, sender);
                    break;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.GET_EVENT_INFO:
                    handleGetEventInfo(request, sendResponse);
                    return true;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.REMOVE_NOTIFICATION_ALARM:
                    handleRemoveNotificationAlarm(request, sendResponse);
                    return true;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.SCHEDULE_NOTIFICATION:
                    handleScheduleNotification(request, sendResponse);
                    return true;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.GET_STORED_NOTIFICATIONS:
                    handleGetStoredNotifications(sendResponse);
                    return true;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.UPDATE_TRADE_CONFIRMATION:
                    handleUpdateTradeConfirmation(request, sendResponse);
                    return true;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.CLEAR_CURRENT_EVENT:
                    if (sender.tab?.id) {
                        cleanupTabEventData(sender.tab.id);
                        // Notify popup to update its display
                        chrome.runtime.sendMessage({
                            type: _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.EVENT_CLEARED,
                            data: { tabId: sender.tab.id }
                        });
                    }
                    break;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.EVENT_INITIALIZED:
                    handleEventInitialized(request, sender, sendResponse);
                    return true;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.UPDATE_PRIVACY_MODE:
                    handleUpdatePrivacyMode(request, sendResponse);
                    return true;
                case _types_messages__WEBPACK_IMPORTED_MODULE_3__.MessageType.BROADCAST_PRIVACY_MODE:
...
function handleEventInitialized(request, sender, sendResponse) {
    if (sender.tab?.id) {
        (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_1__.log)('Background', `Event initialized for tab ${sender.tab.id}:`, request.data);
        chrome.storage.local.set({ [`currentEvent_${sender.tab.id}`]: request.data });
        _store_store__WEBPACK_IMPORTED_MODULE_2__.useStore.getState().addEvent(request.data);
        sendResponse({ success: true });
    }
...
function handleUpdatePrivacyMode(request, sendResponse) {
    const { enabled } = request.data;
    chrome.storage.local.set({ privacyModeEnabled: enabled }, () => {
        (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_1__.log)('Background', `Privacy mode ${enabled ? 'enabled' : 'disabled'}`);
        sendResponse({ success: true });
        // Notify all tabs about the change
        chrome.tabs.query({ url: "https://*.polymarket.com/*" }, (tabs) => {
            tabs.forEach(tab => {
...
function handleBroadcastPrivacyMode(request, sendResponse) {
    const { enabled } = request.data;
        ...
        tabs.forEach(tab => {
            if (tab.id && tab.id !== request.sender?.tab?.id) {
                chrome.tabs.sendMessage(tab.id, {
                    ...
                    (0,_utils_logUtils__WEBPACK_IMPORTED_MODULE_1__.log)('Background', 'Error sending privacy mode update to tab:', error);
                });
            }
        });
    });
